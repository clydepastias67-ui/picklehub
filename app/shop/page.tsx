...
export default function ShopPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const { data } = await supabase.from('products').select('*').eq('is_active', true).gt('stock', 0).order('category');
      setProducts(data || []);
      setLoading(false);
    };
    fetchData();
    ...
    const refreshProducts = async () => {
      const { data } = await supabaseRt.from('products').select('*').eq('is_active', true).gt('stock', 0).order('category');
